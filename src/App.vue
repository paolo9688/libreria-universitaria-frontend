  ...
  <div id="app">
      ...
      <b-container class="d-flex align-items-center">
        ...
        <b-collapse id="nav-collapse" is-nav>
          <b-navbar-nav class="ml-auto align-items-center">
            <b-nav-item
              v-if="isAuthenticated && currentPage === 'bookList'"
              @click="currentPage = 'manageBooks'"
              class="text-white"
            >
              Gestione Libri
            </b-nav-item>
            <b-nav-item
              v-if="isAuthenticated && currentPage === 'manageBooks'"
              @click="currentPage = 'bookList'"
              class="text-white"
            >
              Lista Libri
            </b-nav-item>
            <b-nav-item v-if="isAuthenticated" @click="handleLogout" class="text-white">
              Logout
            </b-nav-item>
            <b-nav-item v-else @click="showRegisterForm = false" class="text-white">
              Login
            </b-nav-item>
            <b-nav-item
    ...
    <main class="app-main-content">
      <template v-if="isAuthenticated">
        <LibriManager
          @showBookList="currentPage = 'bookList'"
          v-if="currentPage === 'manageBooks'"
        />
        <BookList
          @goToManageBooks="currentPage = 'manageBooks'"
          v-else-if="currentPage === 'bookList'"
        />
      </template>
      <template v-else>
        <!-- Wrapper aggiornato per Login/Register -->
        <div class="auth-wrapper d-flex justify-content-center">
          <Login
            @loginSuccess="handleLoginSuccess"
            @goToRegister="showRegisterForm = true"
            v-if="!showRegisterForm"
          />
          <Register
            @registrationSuccess="handleRegistrationSuccess"
            @goToLogin="showRegisterForm = false"
            v-else
          />
        </div>
      </template>
...
const currentPage = ref<string>('manageBooks');
const currentYear = ref<number>(new Date().getFullYear());

const checkAuthStatus = () => {
  isAuthenticated.value = AuthService.isAuthenticated();
  currentPage.value = isAuthenticated.value ? 'manageBooks' : 'login';
};

const handleLoginSuccess = () => {
  checkAuthStatus();
  currentPage.value = 'manageBooks';
};
...
/* ✅ Wrapper per login/register per riempire l'altezza */
.auth-wrapper {
  flex-grow: 1;
  width: 100%;
  min-height: 100%;
